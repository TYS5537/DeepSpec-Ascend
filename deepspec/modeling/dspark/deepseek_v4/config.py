...
def build_draft_config(target_config, model_args):
    assert target_config.model_type == "deepseek_v4", (
        "DeepSeek-V4 DSpark expects a DeepSeek-V4 target config, "
    ...
    # DFlash does not use DeepSeek-V4's target compressor schedule inside the
    # small draft model; eager sliding attention keeps the DSpark mask exact.
